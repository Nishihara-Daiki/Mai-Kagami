...
SongInformation::SongInformation(Font *font, Songs *songs, Touch *touch, User *user) {
	...
	grad[0] = new MyDrawGraph(WIDTH * 0.5, HEIGHT * 0.22-30, "img/grad1.png");	grad[0]->SetExceptFadeFlag(TRUE);
	grad[1] = new MyDrawGraph(WIDTH * 0.5, HEIGHT * 0.8, "img/grad2.png");		grad[1]->SetExceptFadeFlag(TRUE);
	...
	case MAIN:
		if (nowScene == lastScene) {
			...
			if(touch->Hold2(0)) {
				direct = 1;  // Jaity
				for (int i = 0; i < n; i++) {
					songCover[i]->coverGraph->Stop(FALSE, TRUE);
					songCover[i]->coverWhite->Stop(FALSE, TRUE);
				}
			}
