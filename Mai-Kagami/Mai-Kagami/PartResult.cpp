...
PartResult::PartResult(Font *font, Songs *songs, Touch *touch) {
	this->songs = songs;
	this->font = font;
	title = new MyDrawTextLine(font, "Ì“_Œ‹‰Ê", WIDTH * 0.5, HEIGHT * 0.15, 1, 60, WIDTH * 0.5, 4, &sceneOpacity);
	button = new CircleButton2(font, touch, "ŽŸ‚Ö", 4, &sceneOpacity);
}
...
PartFinish::PartFinish(Font *font, Touch *touch) {
	...
	button[0] = new CircleButton(font, touch, "‚à‚¤ˆê“x", 0, WIDTH * 0.75, &sceneOpacity, "White");
	button[1] = new CircleButton(font, touch, "•”•ª—ûK", 1, WIDTH * 0.75, &sceneOpacity, "White");
	button[2] = new CircleButton(font, touch, "’Ê‚µ—ûK", 2, WIDTH * 0.75, &sceneOpacity, "White");
	button[3] = new CircleButton(font, touch, "‹È‘I‘ð‰æ–Ê", 3, WIDTH * 0.75, &sceneOpacity, "White");
}
